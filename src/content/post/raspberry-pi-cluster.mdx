...
| ------------ | -------- | ---------------------------------------------- | ----------------- | ----------------- | ------------- | ------------------------------------------------------------------------------------------------- |
| pi-grey      | HEAD     | <div>192.168.0.3</div> <div>192.168.50.1</div> | DC:A6:32:7D:8B:2A | e7a04c41          | USB           | [USB & Ethernet](https://thepihut.com/products/usb-3-2-hub-gigabit-ethernet-hat-for-raspberry-pi) |
| pi-orange    | CLUSTER  | 192.168.50.11                                  | DC:A6:32:31:39:61 | 60c90130          | NET           | [PoE+](https://thepihut.com/products/raspberry-pi-poe-plus-hat)                                   |
| pi-red       | CLUSTER  | 192.168.50.12                                  | E4:5F:01:63:8C:02 | 1862a49b          | NET           | [PoE+](https://thepihut.com/products/raspberry-pi-poe-plus-hat)                                   |
| pi-yellow    | CLUSTER  | 192.168.50.13                                  | E4:5F:01:1F:58:56 | 2f9942e8          | NET           | [PoE+](https://thepihut.com/products/raspberry-pi-poe-plus-hat)                                   |
...
<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ sudo apt install isc-dhcp-server`}
/>
...
<Prism
  lang="shellsession"
  code={`DHCPDv4_CONF=/etc/dhcp/dhcpd.conf
...
<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ sudo apt install nfs-kernel-server`}
/>
...
<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ sudo mkdir /network-share
...
<Prism lang="shellsession" code={`/network-share 192.168.50.0/24(rw,sync)`} />
...
<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ sudo systemctl enable rpcbind.service
...
<Prism lang="shellsession" code={`tim@pi-grey:~ $ sudo reboot`} />
...
<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ sudo apt install tftpd-hpa
...
<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ sudo systemctl restart tftpd-hpa`}
/>
...
<Prism
  lang="shellsession"
  code={`root@pi-grey:/tmp/image # touch /network-boot/pi-orange/boot/ssh
...
<Prism
  lang="shellsession"
  code={`tim:$6$Pp1bRRnPBm3wcbG.$LYUGp3j7olEbbIdPAD/1Btfg3QOIOVnUzjFTKag2Jb0p84zLCHBfD5kokmEUMCG6Fcyu4Y8oh3T94vzuj4ama0`}
/>
...
<Prism
  lang="shellsession"
  code={`root@pi-grey:/tmp/image # echo "/network-boot/pi-orange/boot /network-boot/tftp/60c90130 none defaults,bind 0 0" >> /etc/fstab
...
<Prism
  lang="shellsession"
  code={`root@pi-grey:/tmp/image # systemctl restart rpcbind
...
<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ ssh 192.168.50.11
...
<Prism
  lang="shellsession"
  code={`tim@pi-orange:~ $ sudo systemctl disable resize2fs_once.service
...
<Prism lang="shellsession" code={`tim@pi-orange:~ $ sudo raspi-config`} />
...
<Prism
  lang="shellsession"
  code={`tim@pi-orange:~ $ sudo mkdir /network-share
...
<Prism
  lang="shellsession"
  code={`192.168.50.1:/network-share /network-share nfs defaults 0 0`}
/>

Then reboot the compute node.

<Prism lang="shellsession" code={`tim@pi-orange:~ $ sudo reboot`} />
...
<Prism
  lang="shellsession"
  code={`tim@pi-orange:~ $ sudo systemctl restart ssh`}
/>
...
<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ ssh-keygen -t rsa -b 4096 -C "tim@pi-cluster"
...
<Prism lang="shellsession" code={`net.ipv4.ip_forward=1`} />

After activating forwarding we’ll need to configure `iptables`:

<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ apt install iptables
...
<Prism
  lang="shellsession"
  code={`console=serial0,115200 console=tty root=/dev/nfs nfsroot=192.168.50.1:/network-boot/pi-red,vers=4.1,proto=tcp rw ip=dhcp rootwait`}
/>

and similarly for `/network-boot/pi-red/etc/hostname`.

<Prism lang="shellsession" code={`pi-red`} />
...
<Prism
  lang="shellsession"
  code={`console=serial0,115200 console=tty root=/dev/nfs nfsroot=192.168.50.1:/network-boot/pi-yellow,vers=4.1,proto=tcp rw ip=dhcp rootwait`}
/>

and similarly for `/network-boot/pi-yellow/etc/hostname`.

<Prism lang="shellsession" code={`pi-yellow`} />
...
<Prism lang="shellsession" code={`$ sudo reboot`} />
...
<Prism
  lang="shellsession"
  code={`tim@pi-grey:~ $ nmap 192.168.50.0/24
